...
station_rollouts[order(station_rollouts$community_area), ]

library(mapview)
mapview(station_rollouts, zcol = "capacity")
```
...
cook_base +
  geom_point(aes(x= lon, y=lat, group = rollout_year, color = rollout_month), 
             data = station_rollouts, alpha = 0.5) +
  facet_wrap(~rollout_year)+
  theme(legend.position = "right")
```
